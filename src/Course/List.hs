...
find ::
  ...
  -> List a
  -> Optional a
find p = headOr Empty . (foldRight (\x acc -> if p x then (Full x) :. acc else acc) Nil) 
...
reverse ::
  ...
  -> List a
--reverse Nil = Nil 
--reverse (x :. xs) = let la = reverse xs in la ++ (x :. Nil)
